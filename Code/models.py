...
from sklearn.model_selection import train_test_split, HalvingGridSearchCV # type: ignore
import numpy as np
import pandas as pd
...
class Models_pollen():
    ...
    # Normal partition taking into account the quantity of moments o photographies
    def Partition_unbalanced(self,rate=0.75):
        try:
            # Partition by de moments
            ...
            # Allow to enjoy the data of the same producer in one of the groups without caring of the photo
    ...
    # Partition related with the sample frequency
    def Partition_balanced(self, cant_visitas: int, rate_split=2/3):
        ...
        pos=[]
        pos_val=[]
        # Union of the differents position of the photographies of each sample
        for k,j in zip(pos0,pos1):
            pos.append(k)
            pos.append(j)
        Positions.append(pos)
        train=pos[:cant[0]]# type: ignore
        val=pos[cant[0]:parte[0]] # type: ignore
        pos_val.append(val)
        X_train=X[train]
        X_val=X[val]
        Y_train=Y[train]
        ...
        for i in range(1,conjuntos):
            ...
            for k,j in zip(pos0,pos1):
                ...
                pos.append(j)
                Positions.append(pos)
            train=pos[:cant[i]] # type: ignore
            val=pos[cant[i]:parte[i]] # type: ignore
            pos_val.append(val)
        ...
        columns=["Visit"]+ ["C"+str(i) for i  in range(1,self.groups +1 )]
    ...
    def model_tunning(self, estimator_models: list , params_dict: list[dict], names_model:list[str],
        ...
        results=[]
        # Tunning parameters by each metric
        for score in score_list:
            
            # Tunning parameters by each model
