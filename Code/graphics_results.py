...
class Graphic_results():
    ...
    def get_results(self, y_test:list , pred_y: list, eje: list, save_image: bool):
        
        conf_matrix = confusion_matrix(y_test, pred_y)
        plt.figure(figsize=(7, 6))
        heatmap(conf_matrix, xticklabels=eje, yticklabels=eje, annot=True, fmt="d")
        plt.title("Confusion matrix")
        plt.ylabel('Beekeeper assigned')
        plt.xlabel('Beekeeper predicted')
        plt.rc('axes', labelsize=10)
        plt.xticks(rotation=90, fontsize=7)
        plt.yticks(rotation=0,fontsize=7)
        if save_image:
            plt.savefig("matrizconfusion.jpg",dpi=500,format="jpg",bbox_inches="tight")
        else:
            pass
        plt.show()
