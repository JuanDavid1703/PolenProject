...
class Graphic_results(Cleaning_image):
    ...
    def graphic_score_models(self, name_models, scores: list, Data_name: list ,parameters: list, taman=(10,5), lsize=12):
        score=np.array(scores)
        Data_name=np.array(Data_name)# type: ignore
        parameters=np.array(parameters)# type: ignore
        ...
        for i in range(len(name_models)):
            print("Better accuracy for model: ", np.max(score[:,i]),"Grupo obtenido",
                Data_name[np.where(score[:,i]==np.max(score[:,i]))])# type: ignore
            print("Parámetros: ",parameters[:,i][np.where(score[:,i]==np.max(score[:,i]))][0])# type: ignore
    ...
    def color_print(self, centers_representation, botanical_species: int):
        ...
        if centers_representation.shape[1]==3:# type: ignore
        
            for j in range(botanical_species+1):
                ax[j].set_axis_off()
                ax[j].imshow(cv.cvtColor(np.ones((4,4,3),dtype=np.uint8)*centers_representation[cont],cv.COLOR_HSV2RGB))# type: ignore
                cont +=1

            plt.show()
        
        elif centers_representation.shape[1]==2:# type: ignore
            plt.figure(figsize=(5,5))
            plt.scatter(centers_representation[:,0],centers_representation[:,1], c=range(botanical_species+1))# type: ignore
            plt.show()